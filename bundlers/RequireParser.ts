...
module RequireParser {
     ...
     */
    export function nextToken(i: number, src: string, len: number, dst: { token: string; endedWithSemicolon: boolean }): number {
        var state = 0;
        var token = "";
        while (i < len) {
            var ch = src[i];
            if (ch === '"') {
                ...
                while (i < len) {
                    ch = src[i];
                    if (ch === '\\') state = 8;
                    else if (state === 7 && ch === '"') { token += ch; i++; break; }
                    else state = 7;
                    token += ch;
                    i++;
                }
                ...
                while (i < len) {
                    ch = src[i];
                    if (ch === '\\') state = 10;
                    else if (state === 9 && ch === '\'') { token += ch; i++; break; }
                    else state = 9;
                    token += ch;
                    i++;
                }
     ...
     */
    export function trimParens(str: string) {
        var len = str.length;
        if (len < 2) return str;
