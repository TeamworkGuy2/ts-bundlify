...
function stringConcat(parts: any[]): string {
...
function bufferConcat(parts: Buffer[]): Buffer {
    var bufs: Buffer[] = [];
    for (var i = 0; i < parts.length; i++) {
        var p = parts[i];
...
function arrayConcat(parts: any[]): any[] {
    var res: any[] = [];
    for (var i = 0; i < parts.length; i++) {
...
function u8Concat(parts: any[]): Uint8Array {
    var len = 0;
    for (var i = 0; i < parts.length; i++) {
...
function BufferFrom(value: string, encodingOrOffset?: BufferEncoding): Buffer;
function BufferFrom(value: string | Buffer | ArrayBufferLike, encodingOrOffset?: number | BufferEncoding, length?: number): Buffer;
function BufferFrom(value: string | Buffer | ArrayBufferLike, encodingOrOffset?: number | BufferEncoding, length?: number): Buffer {
