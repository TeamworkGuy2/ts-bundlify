...
suite("ConcatStream", function () {
    ...
    test("typed array stream", function (done) {
        ...
        var arrays = ConcatStream.from({ encoding: "Uint8Array" }, <any>function (out: Uint8Array) {
    ...
    test("typed array from strings, buffers, and arrays", function (done) {
        var arrays = ConcatStream.from({ encoding: "Uint8Array" }, function (out) {
            asr.equal(typeof (<any>out).subarray, "function")
            asr.deepEqual(ConcatStream.bufferFrom(<any>out).toString("utf8"), "abcde fg xyz")
            done();
        });
        arrays.write("abcde");
