...
var ConcatWithSourceMaps = require("../../streams/ConcatWithSourceMaps");
var asr = chai.assert;
suite("ConcatWithSourceMaps", function () {
    function testCase(description, options) {
        test(description, function () {
            ...
            if (options.output.sourceMap) {
                asr.deepEqual(JSON.parse(concat.sourceMap), JSON.parse(options.output.sourceMap), "should produce the right source map");
            }
            else {
    ...
    test('should not allocate an uninitialized buffer when passing a number', function () {
        asr.throws(function () {
            new ConcatWithSourceMaps(true, "all.js", 234);
