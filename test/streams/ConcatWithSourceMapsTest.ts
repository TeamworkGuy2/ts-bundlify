...
import ConcatWithSourceMaps = require("../../streams/ConcatWithSourceMaps");

var asr = chai.assert;

suite("ConcatWithSourceMaps", function () {

    function testCase(description: string, options: { sourceMapping: any; outFile: string; separator?: string; input: { content: string; fileName?: string | null; sourceMap?: string }[]; output: { content: string; sourceMap?: string } }) {
        test(description, function () {
            // content as Buffer
            var concat = new ConcatWithSourceMaps(options.sourceMapping, options.outFile, options.separator);
            options.input.forEach(function (input, i) {
                concat.add(input.fileName !== undefined ? input.fileName : "test" + (i + 1), ConcatWithSourceMaps.bufferFrom(input.content), input.sourceMap);
            });
            asr.equal(concat.content.toString(), options.output.content, "should produce the right output");
            if (options.output.sourceMap) {
                asr.deepEqual(JSON.parse(<any>concat.sourceMap), JSON.parse(options.output.sourceMap), "should produce the right source map");
            }
            else {
                asr.equal(concat.sourceMap, undefined, "should not produce a source map");
            }

            // content as string
            concat = new ConcatWithSourceMaps(options.sourceMapping, options.outFile, options.separator);
            options.input.forEach(function (input, i) {
    ...
    test('should not allocate an uninitialized buffer when passing a number', function () {
        asr.throws(function () {
            new ConcatWithSourceMaps(true, "all.js", <any>234);
